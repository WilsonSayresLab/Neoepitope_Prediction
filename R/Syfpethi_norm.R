...
filename = paste(args[1], "Syfpethi_norm.csv", sep="")
data= read.table(filename,header=T,sep=",")


# SYFPEITHI binding normalization (higher the better)
cmax = min(data$Syfpethi.bind)
cmin = max(data$Syfpethi.bind)
Norm.Syfpethi.bind = ((data$Syfpethi.bind-cmin)/(cmax-cmin))
...
data$Syfpethi.Norm = Norm.Syfpethi.bind
...
filename = paste(args[1], "Syfpethi_normalized.txt", sep="")
