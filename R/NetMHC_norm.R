...
filename = paste(args[1], "NetMHC_norm.csv", sep="")
data= read.table(filename,header=T,sep=",")


# NetMHCpan binding normalization (lower the better)
bmin = min(data$NetMHC.bind)
bmax = max(data$NetMHC.bind)
Norm.NetMHC.bind = ((data$NetMHC.bind-bmin)/(bmax-bmin))
...
data$NetMHC.Norm = Norm.NetMHC.bind
...
filename = paste(args[1] , "NetMHC_normalized.txt", sep="")
